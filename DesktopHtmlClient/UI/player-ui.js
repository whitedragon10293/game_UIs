...
function srcToken(element, value) {
  ...
  else $(element).removeClass("shrink");
}
...
function getSortedPlayerWrappers() {
  ...
  return playerWrappers.sort((a, b) => {
    if (a.classList[1] == undefined || b.classList[1] == undefined) return 0;
...
export class Player {
  ...
  setCards(cards) {
    ...
    for (let i = 0; i < cards.length; ++i) {
      ...
      const tableCard = `<div class="content player-card ${twoCardsClassName}" value=${card} style="animation-name:${
        card == "?" ? "slide-animation" : "slideInDown"
      }">
  ...
  setWrapperField(fieldName, value) {
    ...
    if (!selector) throw `No selector for ${fieldName}`;
  ...
  setDealerButton(visible) {
    if (!visible) this.setWrapperField("dealer", false);
    else this.setWrapperField("dealer", true);
  }

  setSmallBlindButton(visible) {
    if (this.missingSB) return;
    if (!visible) this.setWrapperField("blind", false);
    else this.setWrapperField("blind", smallBlindSrc);
  }

  setBigBlindButton(visible) {
    if (this.missingBB) return;
    if (!visible) this.setWrapperField("blind", false);
    else this.setWrapperField("blind", bigBlindSrc);
  }
  ...
  setMissingBigBlindButton(visible) {
    ...
    if (!visible) this.setWrapperField("missBb", false);
    else this.setWrapperField("missBb", true);
  }
  ...
  setMissingSmallBlindButton(visible) {
    ...
    if (!visible) this.setWrapperField("missSb", false);
    else this.setWrapperField("missSb", true);
  }
  ...
  clearPlayerAnimationCss() {
    ...
    if (lastBetDiv) lastBetDiv.style = "";
  }
  ...
  setTurnTimer(timeout, timeToReact, timeBank) {
    ...
    clonedTimeBar.setAttribute(
      "style",
      `-webkit-animation: timeReactRunOut calc(${
        timeToReact + 1
      }s) steps(500, start);`
    );
    ...
    clonedTimeCircle.setAttribute(
      "style",
      `-webkit-animation: turnCircleReact calc(${
        timeToReact + 1
      }s) steps(500, start);`
    );
    ...
    this.reactTimeOut = setTimeout(() => {
      ...
      if (this.reactTimeOut != undefined) clearTimeout(this.reactTimeOut);
    }, timeToReact * 1000);
  }
